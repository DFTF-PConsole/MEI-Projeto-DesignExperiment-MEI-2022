...
plot_ly(data, x = ~vertices, y = ~probability, z = ~time, color = ~algorithm, colors = "viridis", marker = list(size = 5)) %>% 
  add_markers() %>%
  layout(
...
plot_ly(data, x = ~vertices, y = ~arcs, z = ~time, color = ~algorithm, colors = "viridis", marker = list(size = 5)) %>% 
  add_markers() %>%
  layout(
    title = "Scatter Plot 3D",
    scene = list(
...
plot_ly(data_mean, x = ~vertices, y = ~probability, z = ~time, color = ~algorithm, colors = "viridis", marker = list(size = 5)) %>% 
  add_markers() %>%
  layout(
    scene = list(
      title = "Scatter Plot 3D",
...
plot_ly(data_mean, x = ~vertices, y = ~arcs, z = ~time, color = ~algorithm, colors = "viridis", marker = list(size = 5)) %>% 
  add_markers() %>%
  layout(
    title = "Mean",
    scene = list(
